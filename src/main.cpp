...
#if defined(NDEBUG)
# error "SHREEJI cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x72e12c53e063e72171964e22cadfff76e97e54d37bee6b92c266a11f293001c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SHREEJI: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SHREEJI Signed Message:\n";
...
{
    // SHREEJI: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 840000000 blocks
    nSubsidy >>= (nHeight / 840000000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SHREEJI: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SHREEJI: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1491328210, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "smart village developments with underlying assets digital tokens";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044cec919471c752fb5b3473a9ff4ffd8bb558a4a27f8789767087324101d1b954a9d8423ba84ebb798cbbd9856434f8b60d7980387a0c6218375c67f6b4365364") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1491328210;
        ...
        assert(block.hashMerkleRoot == uint256("0xcafa03ebc2647d6507c68624af1c3ee07953bbc3b99816ce842e192149451862"));
...
unsigned char pchMessageStart[4] = { 0x82, 0x82, 0xe8, 0xf7 }; // SHREEJI: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SHREEJIMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SHREEJIMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SHREEJIMiner(CWallet *pwallet)
{
    printf("SHREEJIMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shreeji-miner");
    ...
    try { loop {
        ...
        printf("Running SHREEJIMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SHREEJIMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SHREEJIMiner, pwallet));
}
